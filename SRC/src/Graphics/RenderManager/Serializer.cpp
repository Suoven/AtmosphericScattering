...
bool RenderManager::LoadScene()
{
	...
	obj->mModel = CustomSphere;
	obj->mScale = glm::vec3(200.0f);
	obj->mPosition = glm::vec3(500.0f, 6370.0f, 0.0f);
	...
	planet->mb_cast_shadow = false;
	planet->color = glm::vec3(0.321f, 0.378f, 0.784f);
	AddRenderable(planet);

	sun = new GameObject();
	sun->mModel = CustomSphere;
	sun->color = glm::vec3(1.0f, 1.0f, 1.0f);
	sun->mb_cast_shadow = false;
	AddRenderable(sun);
	...
	if (j.find("directional_light") != j.end())
	{
		...
		obj2->mPosition = glm::vec3(0.0f, 0.0f, 0.0f) - dir * 15000.0f;
		AddRenderable(obj2);
	}
...
void RenderManager::LoadSphereModel()
{
	...
	sphere_mesh.vertex_count = static_cast<unsigned>(in_positions.size());
