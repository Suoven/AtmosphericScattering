...
#pragma endregion
	...
	GameObject* planet;
	GameObject* sun;
	glm::vec2 sun_rotation = glm::vec2(5.0f, 40.0f);
...
private:
	...
	GLSLProgram* SkyboxRenderProgram = nullptr;
	...
	GLuint						SunRenderFBO, SunRenderText, skyboxText;					//handlers for the Inscatter for the space texture
	...
	float sun_distance = 1000.0f;
	bool use_atmospheric_scatter = true;
	int inscatter_steps = 50;
	float planet_radius  = 6360.0f;
	float top_atmosphere = 60.0f;
	float radius_epsilon = 2.0f;
	glm::vec3 Br  = glm::vec3(0.0058f, 0.0135f, 0.0331f);
	glm::vec3 Bms = glm::vec3(0.02f, 0.02f, 0.02f);
...
private:
	...
	void DrawScene(GLSLProgram* shaderProg, bool use_custom_matrix = false, bool use_textures = true, bool shadow_pass = false);
	void RenderQuad(GLSLProgram* program);

	//different passes in the pipeline
	void SpaceRenderPass();
	void DrawShadowMaps();
	void AmbientPass();
	void DirLightPass();
	void BloomPass();
