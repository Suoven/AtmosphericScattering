...
void GetInput()
{
	...
	if (KeyDown(Key::Q))
	{
		camera->set_position(camera->position() + glm::vec3(0.0f, 1.0f, 0.0f) * RenderMgr.camera_speed * mult);
		camera->set_target(camera->target() + glm::vec3(0.0f, 1.0f, 0.0f) * RenderMgr.camera_speed * mult);
	}
	//move downwards
	if (KeyDown(Key::E))
	{
	...
	if (KeyDown(Key::A))
	{
		camera->set_position(camera->position() + rightaxis);
		camera->set_target(camera->target() + rightaxis);
	}
	//move to the right
	if (KeyDown(Key::D))
	{
	...
	if (KeyDown(Key::W))
	{
		camera->set_position(camera->position() + viewaxis);
		camera->set_target(camera->target() + viewaxis);
	}
	//move backwards
	if (KeyDown(Key::S))
	{
